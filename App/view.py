...
def thread_cycle():
    while True:
        ...
        if int(inputs[0]) == 1:
            ...
            print("Arrival city: "+destination_city+"\n")
            ...
            printREQ2(iata2, catalog)
            distancia = controller.distanceDijkstra(catalog, iata1, iata2)
            print("+++ Dijkstra's Trip details +++")
            print("- Todal distance: "+distancia+" (km)")


        elif int(inputs[0]) == 5:
            init = input("Ingrese la ciudad de origen como IATA: ")
            millas = float(input("Cantidad de millas disponibles: "))
            print("=============== Req No. 4 Inputs ===============")
            print("Departure IATA Code: "+init)
            print("Available Travel Miles: "+str(millas)+'\n')
            print("=============== Req No. 4 Answer ===============")
            print("+++ Departure IATA Code: "+init+" +++")
            printREQ2(init, catalog)
            km = float(millas)*1.6
            num, kilometers, path = controller.mst(catalog, km, init)
            print("- Number of possible airports: "+str(num))
            print("- Traveling distance sum between airports: "+str(round(kilometers, 2))+" (km)")
            print("- Passenger available traveling miles: "+str(km)+" (km)")
            print("The longest possible route with airport "+init+" is:", path)
